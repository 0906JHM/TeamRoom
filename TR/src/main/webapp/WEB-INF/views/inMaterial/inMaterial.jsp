	...
	<div id="con">
		...
		<div id="searchForm">
			...
			<input type="text" name="clientCompany" id="clientCompany9999"
				placeholder="거래처를 선택하세요."> <input type="button" value="검색"
				id="searchButton" onclick="searchItem('client','clientCode9999')">
		</div>
		<hr>
		<div id="buttons">
			<input type="button" value="전체" id="allButton"> <input
				type="button" value="입고" id="inButton"> <input type="button"
				value="미입고" id="non_inButton">
		</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(
				function() {
					...
					$("#oinNum, #rawName9999, #clientCompany9999").on(
							'keydown', function(e) {
								if (e.key === 'Enter') {
	    ...
	    function updateinMaterialList(data) {
	        ...
	        for (var i = 0; i < data.length; i++) {
        		...
        		if(i < data.length-1){
		            ...
		            var row = $("<tr>");
		            row.append("<td>" + (data[i].inNum ? data[i].inNum : '-') + "</td>");
		            row.append("<td>" + (data[i].buyNum ? data[i].buyNum : '-') + "</td>");
		            row.append("<td>" + (data[i].whseCode ? data[i].whseCode : '-') + "</td>");
		            row.append("<td>" + (data[i].clientCompany ? data[i].clientCompany : '-') + "</td>");
		            row.append("<td>" + (data[i].rawCode ? data[i].rawCode : '-') + "</td>");
		            row.append("<td>" + (data[i].rawName ? data[i].rawName : '-') + "</td>");
		            row.append("<td>" + (data[i].inCount ? data[i].inCount : '-') + "</td>");
		            row.append("<td>" + (data[i].stockCount ? data[i].stockCount : '-') + "</td>");
		            row.append("<td>" + (data[i].rawPrice ? data[i].rawPrice : '-') + "</td>");
		            row.append("<td>" + (data[i].inPrice ? formatCurrency(data[i].inPrice) : '-') + "</td>");
		            row.append("<td>" + (data[i].inDate ? data[i].inDate : '-') + "</td>");
		            row.append("<td>" + (data[i].inEmpId ? data[i].inEmpId : '-') + "</td>");
		            row.append("<td>" + (data[i].inState ? data[i].inState : '-') + "</td>");
		            ...
		            var contextPath = "${pageContext.request.contextPath}";
	  				...
	  				(function(outCode) {
	  			        ...
	  			        button.click(function () {
	  			            ...
	  			            window.open(contextPath + "/inMaterial/inMaterialContent?inNum=" + inNum, "입고 상세정보", "width=500,height=800,toolbar=no,location=no,resizable=yes");
	  			        });
